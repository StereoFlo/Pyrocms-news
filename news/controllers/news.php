...
class News extends Public_Controller
{
    ...
    {
        $pagination = create_pagination('news/page', $this->news_m->count_by(['status' => 'live']), $this->limit, 3);
        $news = $this->news_m->limit($pagination['limit'])->get_many_by(['status' => 'live']);
    ...
    public function category($slug = '')
    {
        ...
        $this->data->pagination = create_pagination('news/category/' . $slug, $this->news_m->count_by([
            'category' => $slug,
            'status' => 'live'
        ]), $this->limit, 4);
        $this->data->news = $this->news_m->limit($this->data->pagination['limit'])->get_many_by([
            'category' => $slug,
            'status' => 'live'
        ]);
    ...
    public function archive($year = NULL, $month = '01')
    {
        ...
        $this->data->pagination = create_pagination('news/archive/' . $year . '/' . $month, $this->news_m->count_by(['year' => $year, 'month' => $month]), $this->limit, 5);
        $this->data->news = $this->news_m->limit($this->data->pagination['limit'])->get_many_by(['year' => $year, 'month' => $month]);
    ...
    public function view($slug = '')
    {
        ...
        $this->news_m->update($article->id, ['views' => ($this->news_m->get($article->id)->views + 1)]);
     ...
     * @return array
     */
    private function _articles_metadata(&$articles = [])
    {
        $keywords = [];
        $description = [];

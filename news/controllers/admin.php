...
class Admin extends Admin_Controller
{
	...
	{
		$total_rows = $this->news_m->count_by([
		    'show_future' => true,
            'status' => 'all']);
		$pagination = create_pagination('admin/news/index', $total_rows);
		$news = $this->news_m->limit($pagination['limit'])->get_many_by([
			'show_future' => TRUE,
			'status' => 'all'
        ]);
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->input->post()) //$this->form_validation->run()
        {
            ...
            if ($result) {
                $this->session->set_flashdata(['success' => sprintf($this->lang->line('news_edit_success'), $this->input->post('title'))]);
            } else {
